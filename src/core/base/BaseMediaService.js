...
class BaseMediaService extends BaseService {
        ...
        return this._executeWithErrorHandling(async () => {
            ...
            if (!media) {
                ...
                throw new NotFoundError(
                    `${this.getMediaType()} with ID ${externalId} not found`
                );
            }
    ...
    _shouldSync(media) {
        ...
        if (isStale) {
            const daysSinceSync = this._daysBetween(
                new Date(media.lastSyncedAt), 
                new Date()
            );
    ...
    async _syncFromExternal(externalId, existingMedia = null) {
        try {
            ...
            if (existingMedia) {
                this._logWarn(
                    `Sync failed for ${this.getMediaType()}, using cached data`,
                    { 
                        externalId, 
                        error: error.message,
                        cacheAge: this._daysBetween(
                            new Date(existingMedia.lastSyncedAt),
                            new Date()
                        )
                    }
                );
    ...
    async _getFromDatabase(externalId) {
        try {
            const idField = this.getExternalIdField();
    ...
    async getTrending(page = 1, perPage = 20) {
        ...
        return this._executeWithErrorHandling(async () => {
            ...
            const trendingData = await this.externalClient.getTrending(
                this.getMediaType(),
                pagination
            );
